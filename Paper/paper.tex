...
Deze maskers gebruiken we om bij het opstellen van de collision matrix. De cellen in deze matrix horen bij een paar van sequenties. De waarde in de cel geeft weer hoe goed beide sequenties op elkaar lijken. We beginnen met een lege collision matrix. Vervolgens nemen we het eerste masker en dekken er de SAX-woorden van alle sequenties mee af. Dan kijken we naar elke paar sequenties en indien de gemaskeerde woorden hetzelfde zijn, incrementeren we de waarde in de bijhorende cel. Dit herhalen we voor elk masker. Daarna kijken we naar de waarden in de gevulde collision matrix. Voor elke cel met een waarde groter dan of gelijk aan de collision threshold komt het bijhorende paar in aanmerking als mogelijke match. Deze collision threshold hebben we de waarde 7 gegeven. Elk paar sequentes dat in aanmerking komt om een match te zijn, wordt dan eindelijk echt vergeleken. Dit gebeurt door de euclidische afstand tussen de genormaliseerde sequenties te berekenen. Pas als deze afstand kleiner is dan een vooraf gedefinieerde variabele $range$, spreken we van een match. Wij hebben deze $range$ de waarde 5 gegeven.
