...
class DynamicTimeSeq(object):
    ...
    def orderMotifs(self, motifs):
        ...
        for motif in motifs:
            ...
            for match,afstand in self.getMotifs()[motif]:
                ...
                raise RuntimeError("Er zijn te weinig goede motieven")
    ...
    def getBestMotifs(self, nbMotifs):
        mostMatchesMotifs = [i for i in self.motifs if len(self.motifs[i]) == self.numberOfGroups - 1]
        motifs = sorted(mostMatchesMotifs,key=lambda motif: self.getTotalDistance(self.motifs[motif]))
        it = iter(motifs)
        bestMotifs = []
